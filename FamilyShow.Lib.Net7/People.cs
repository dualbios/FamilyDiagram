...
public class ContentChangedEventArgs : EventArgs
{
    ...
    public Person NewPerson => newPerson;
...
public class People
{
    ...
    /// </summary>
    public PeopleCollection PeopleCollection => peopleCollection;
    ...
    public void Save()
    {
        ...
        if (PeopleCollection == null || PeopleCollection.Count == 0) return;
        ...
        if (string.IsNullOrEmpty(FullyQualifiedFilename)) FullyQualifiedFilename = DefaultFullyQualifiedFilename;
    ...
    public void LoadVersion2()
    {
        ...
        try
        {
            ...
            using (Stream stream = new FileStream(FullyQualifiedFilename, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                foreach (Person person in people.PeopleCollection) PeopleCollection.Add(person);
                ...
                foreach (Person person in PeopleCollection)
                {
                    ...
                    foreach (Relationship relationship in person.Relationships) relationship.RelationTo = PeopleCollection.Find(relationship.PersonId);

                    // store the stories into temp directory to be packaged into OPC later
                    foreach (Photo photo in person.Photos)
                    {
                        var photoOldPath = Path.Combine(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), App.ApplicationFolderName), photo.RelativePath);
                        if (File.Exists(photoOldPath))
                        {
                            var photoFile = Path.Combine(photoFolder, Path.GetFileName(photo.FullyQualifiedPath));
                    ...
                    {
                        var storyOldPath = Path.Combine(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), App.ApplicationFolderName), person.Story.RelativePath);
                        if (File.Exists(storyOldPath))
                        {
                            var storyFile = Path.Combine(storyFolder, Path.GetFileName(person.Story.AbsolutePath));
    ...
    public static void RecreateDirectory(string folderToDelete)
    {
        try
        {
            if (Directory.Exists(folderToDelete)) Directory.Delete(folderToDelete, true);
...
public class PeopleCollection : ObservableCollection<Person>, INotifyPropertyChanged
{
    ...
    {
        dirty = true;
        if (ContentChanged != null) ContentChanged(this, new ContentChangedEventArgs(null));
    }
    ...
    {
        dirty = true;
        if (ContentChanged != null) ContentChanged(this, new ContentChangedEventArgs(newPerson));
    }
    ...
    public void AddSibling(Person person, Person sibling)
    {
        ...
        if (!Contains(sibling)) Add(sibling);
    }
