...
public class GedcomExport
{
    ...
    private void ExportFamilies()
    {
        ...
        foreach (Family family in map.Values) ExportFamily(family);
    }
    ...
    private void ExportFamily(Family family)
    {
        ...
        if (family.ParentRight == null && family.Children.Count == 0) return;
        ...
        // Children.
        foreach (Person child in family.Children) WriteLine(1, "CHIL", string.Format(CultureInfo.InvariantCulture, "@{0}@", idMap.Get(child.Id)));
    }
    ...
    {
        // PartnerLeft.
        if (partnerLeft != null && partnerLeft.Gender == Gender.Male) WriteLine(1, "HUSB", string.Format(CultureInfo.InvariantCulture, "@{0}@", idMap.Get(partnerLeft.Id)));

        if (partnerLeft != null && partnerLeft.Gender == Gender.Female) WriteLine(1, "WIFE", string.Format(CultureInfo.InvariantCulture, "@{0}@", idMap.Get(partnerLeft.Id)));

        // PartnerRight.
        if (partnerRight != null && partnerRight.Gender == Gender.Male) WriteLine(1, "HUSB", string.Format(CultureInfo.InvariantCulture, "@{0}@", idMap.Get(partnerRight.Id)));

        if (partnerRight != null && partnerRight.Gender == Gender.Female) WriteLine(1, "WIFE", string.Format(CultureInfo.InvariantCulture, "@{0}@", idMap.Get(partnerRight.Id)));
        ...
        if (relationship.SpouseModifier == SpouseModifier.Current)
        {
            ...
            // Date if it exist.
            if (relationship.MarriageDate != null) WriteLine(2, "DATE", relationship.MarriageDate.Value.ToShortDateString());
        }
    ...
    {
        var firstMiddleNameSpace = !string.IsNullOrEmpty(person.FirstName) && !string.IsNullOrEmpty(person.MiddleName) ? " " : "";

        var value = string.Format(CultureInfo.InvariantCulture, "{0}{1}{2}/{3}/", person.FirstName, firstMiddleNameSpace, person.MiddleName, person.LastName);
    ...
    private void ExportEvent(string tag, DateTime? date, string place)
    {
        ...
        if (date == null && string.IsNullOrEmpty(place)) return;

        // Start the new event tag.
        WriteLine(1, tag, "");

        // Date.
        if (date != null) WriteLine(2, "DATE", date.Value.ToShortDateString());

        // Place.
        if (!string.IsNullOrEmpty(place)) WriteLine(2, "PLAC", place);
    }
    ...
    private void WriteLine(int level, string tag, string value)
    {
        ...
        for (var lineIndex = 0; lineIndex < lines.Length; lineIndex++)
        {
            ...
            var chunkCount = (line.Length + ValueLimit - 1) / ValueLimit;

            for (var chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++)
            {
                // Current position in the value.
                var pos = chunkIndex * ValueLimit;

                // Current value chunk to write.
                var chunk = line.Substring(pos, Math.Min(line.Length - pos, ValueLimit));
