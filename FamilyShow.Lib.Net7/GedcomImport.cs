...
public class GedcomImport
{
    ...
    public void Import(PeopleCollection peopleCollection, Stream stream)
    {
        ...
        try
        {
            ...
            if (peopleCollection.Count > 0) peopleCollection.Current = peopleCollection[0];
        }
        catch (Exception ex)
        {
    ...
    private void ImportFamilies()
    {
        ...
        foreach (XmlNode node in list)
        {
            ...
            var husband = GetHusbandID(node);
            var wife = GetWifeID(node);
            ...
            foreach (var child in children)
            {
                ...
                if (husbandPerson != null && childPerson != null) RelationshipHelper.AddChild(people, husbandPerson, childPerson);

                if (husbandPerson == null && (wifePerson != null) & (childPerson != null)) RelationshipHelper.AddChild(people, wifePerson, childPerson);
            }
        }
    }
    ...
    private static void ImportMarriage(Person husband, Person wife, XmlNode node)
    {
        ...
        {
            // Get dates.
            var marriageDate = GetValueDate(node, "MARR/DATE");
            var divorceDate = GetValueDate(node, "DIV/DATE");
            SpouseModifier modifier = GetDivorced(node) ? SpouseModifier.Former : SpouseModifier.Current;
            var Marriageplace = GetValue(node, "MARR/PLAC");
    ...
    private static void FillContact(Contact contact, XmlNode node)
    {
        ...
        if (!string.IsNullOrEmpty(valeur)) contact.Phone = valeur;
    }
    ...
    private static void ImportNote(Person person, XmlNode node)
    {
        try
        {
            var value = GetValue(node, "NOTE");
            if (!string.IsNullOrEmpty(value))
            {
                person.Story = new Story();
                var storyFileName = new StringBuilder(person.Name).Append(".rtf").ToString();
    ...
    private static string[] GetPhotos(XmlNode node)
    {
        ...
        for (var i = 0; i < list.Count; i++) photos[i] = GetFile(list[i]);
    ...
    {
        var value = GetValue(node, "SEX");
        if (string.Compare(value, "f", true, CultureInfo.InvariantCulture) == 0) return Gender.Female;
    ...
    {
        var value = GetValue(node, "DIV");
        if (string.Compare(value, "n", true, CultureInfo.InvariantCulture) == 0) return false;
    ...
    private static string[] GetChildrenIDs(XmlNode node)
    {
        ...
        for (var i = 0; i < list.Count; i++) children[i] = GetId(list[i]);
    ...
    private static DateTime? GetValueDate(XmlNode node, string xpath)
    {
        ...
        try
        {
            var value = GetValue(node, xpath);
            if (!string.IsNullOrEmpty(value)) result = DateTime.Parse(value, CultureInfo.InvariantCulture);
        }
        catch
        {
    ...
    private static string GetValueId(XmlNode node, string xpath)
    {
        ...
        try
        {
            ...
            if (valueNode != null) result = valueNode.Attributes["Value"].Value.Replace('@', ' ').Trim();
        }
        catch
        {
            // Invalid line, keep processing the file.
    ...
    private static string GetValue(XmlNode node, string xpath)
    {
        ...
        {
            XmlNode valueNode = node.SelectSingleNode(xpath);
            if (valueNode != null) result = valueNode.Attributes["Value"].Value.Trim();
        }
        catch
        {
            // Invalid line, keep processing the file.
