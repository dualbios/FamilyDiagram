...
public static class RelationshipHelper
{
    ...
    public static void AddChild(PeopleCollection family, Person person, Person child)
    {
        ...
        foreach (Person existingSibling in person.Children) family.AddSibling(existingSibling, child);
    ...
    public static void AddParent(PeopleCollection family, Person person, Person parent)
    {
        ...
        if (person.Parents.Count == 2) return;
        ...
        if (person.Parents != null)
            switch (person.Parents.Count)
            {
                ...
                case 2:
                    ...
                    foreach (Person parent in person.Parents) family.AddChild(parent, sibling, ParentChildModifier.Natural);
    ...
    {
        for (var i = person.Relationships.Count - 1; i >= 0; i--)
    ...
    {
        if (!personToDelete.IsDeletable) return;
