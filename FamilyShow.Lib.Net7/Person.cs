...
public class Person : INotifyPropertyChanged, IEquatable<Person>, IDataErrorInfo
{
    ...
    public int? Age
    {
        get
        {
            ...
            if (endDate.Month < startDate.Month || (endDate.Month == startDate.Month && endDate.Day < startDate.Day)) age--;
    ...
    /// </summary>
    public PhotoCollection Photos => photos;
    ...
    public bool IsDeletable
    {
        get
        {
            ...
            if (relationships.Count < 3)
            {
                ...
                if (Spouses.Count == 2) return false;

                // The person is connecting two generations
                if (Parents.Count == 1 && Children.Count == 1) return false;

                // The person is connecting inlaws
                if (Parents.Count == 1 && Spouses.Count == 1) return false;
            ...
            if (Children.Count > 0 && Parents.Count == 0 && Siblings.Count == 0 && Spouses.Count == 0) return true;
            ...
            if (Siblings.Count > 0 && Parents.Count >= 0 && Spouses.Count == 0 && Children.Count == 0) return true;
    ...
    /// </summary>
    public RelationshipCollection Relationships => relationships;
        ...
        get
        {
            ...
            foreach (Relationship relationship in relationships)
                if (relationship.RelationshipType == RelationshipType.Spouse)
                {
                    ...
                    if (spouseRel != null && spouseRel.SpouseModifier == SpouseModifier.Current) spouses.Add(relationship.RelationTo);
                }
        ...
        get
        {
            ...
            foreach (Relationship rel in relationships)
                if (rel.RelationshipType == RelationshipType.Spouse)
                {
                    ...
                    if (spouseRel != null && spouseRel.SpouseModifier == SpouseModifier.Former) spouses.Add(rel.RelationTo);
                }
    ...
    public Collection<Person> Children
    {
        get
        {
            Collection<Person> children = new();
            foreach (Relationship relationship in relationships)
    ...
    public Collection<Person> Siblings
    {
        get
        {
            Collection<Person> siblings = new();
            foreach (Relationship relationship in relationships)
    ...
    public Collection<Person> HalfSiblings
    {
        get
        {
            ...
            Collection<Person> halfSiblings = new();
    ...
    public ParentSetCollection PossibleParentSets
    {
        get
        {
            ParentSetCollection parentSets = new();

            foreach (Person parent in Parents)
            foreach (Person spouse in parent.Spouses)
            {
                ParentSet parentSet = new(parent, spouse);

                // Don't add the same parent set twice.
                if (!parentSets.Contains(parentSet)) parentSets.Add(parentSet);
            }
    ...
    public ParentSetCollection MakeParentSets()
    {
        ...
        foreach (Person spouse in Spouses)
        {
            ...
            // Don't add the same parent set twice.
            if (!parentSets.Contains(ps)) parentSets.Add(ps);
        }
    ...
    public void DeletePhotos()
    {
        ...
        foreach (Photo photo in photos) photo.Delete();
    }
