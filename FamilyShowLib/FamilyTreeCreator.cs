...
namespace Microsoft.FamilyShowLib
{
    ...
    {
        private readonly List<DiagramItem> families = new List<DiagramItem>();

        public IEnumerable<DiagramItem> CreateTree(PeopleCollection peopleCollection, Person current)
        {
            DiagramItem grandPaItem = new DiagramItem() { Persons = current.Parents[0].Parents, Generation = -2 };
            DiagramItem grandMaItem = new DiagramItem() { Persons = current.Parents[1].Parents, Generation = -2 };
            families.Clear();
                ...
                if (!di.Children.Contains(child))
                {
                    ...
                    if (child.Spouses.Any())
                    {
                        foreach (Person spouse in child.Spouses)
                        {
                            ...
                            {
                                DiagramItem dd = new DiagramItem()
                                {
